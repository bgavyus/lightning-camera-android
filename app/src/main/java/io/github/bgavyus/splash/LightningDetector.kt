...
class LightningDetector(context: Context, textureView: TextureView, videoSize: Size) {
	...
	private var mInputBitmap = Bitmap.createBitmap(videoSize.width, videoSize.height, Bitmap.Config.ARGB_8888, false)
	private var mInputAllocation = Allocation.createFromBitmap(renderScript, mInputBitmap)
	private var mOutputAllocation = Allocation.createSized(renderScript, Element.I32(renderScript), mHistogram.size)
	private var mHistogramKernel = ScriptIntrinsicHistogram.create(renderScript, Element.U8_4(renderScript)).apply { setOutput(mOutputAllocation) }
